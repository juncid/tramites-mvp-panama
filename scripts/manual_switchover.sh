...
docker-compose -f config/docker-compose.green-blue.yml stop backend-green

# Paso 3: Cambiar puerto de BLUE al principal (8000)
echo "🔄 Reconfigurando puertos para BLUE..."

# Detener BLUE temporalmente
docker-compose -f config/docker-compose.green-blue.yml stop backend-blue
...
docker-compose -f config/docker-compose.switchover.yml up -d backend-blue-active
...
if [ "$MAIN_STATUS" = "blue-active" ]; then
    ...
    docker-compose -f config/docker-compose.switchover.yml down
    docker-compose -f config/docker-compose.green-blue.yml up -d backend-green
    exit 1
fi